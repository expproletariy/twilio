...
func NewHttpErrorNotUpdated() HttpError {
	return &httpError{msg: "can not update resource", status: http.StatusBadRequest}
}

func NewHttpErrorNotUpdatedWithResource(resource string) HttpError {
	return &httpError{msg: "can not update resource", status: http.StatusBadRequest, resource: resource}
}
